...
- **Data processing scripts**:
  - `usftt_results.py` - Fetches player data, calculates progressions, generates CSV to `data/` folder
  - `usftt_results_teams.py` - Fetches team data, match results, and team rankings to `data/` folder
- **Testing**: `tests/` directory with pytest (34 tests covering division normalization, team ID extraction with gender detection, and ranking API)
- **Data storage**: `data/` folder contains CSV files for players, licenses, competitors, and team results (tracked in git)
...
3. Data is saved to CSV files in the `backend/data/` directory (tracked in git)
...
- `backend/` - Backend Python scripts and data processing
  - `data/` - CSV data files (generated by scripts, tracked in git)
- `docs/` - Documentation
- `copy-csv.sh` - Helper script to copy CSV files from `backend/data/` to `frontend/public/backend/`

**Notes**:
- CSV data files are generated by backend scripts and stored in `backend/data/` (tracked in git)
...
- **Team ID extraction**: Generates team IDs with gender markers (F/G) by detecting "Dames" or "Feminine" keywords in division names
...
**Test Files**:
- `tests/test_usftt_results_teams.py` - 34 tests covering:
  - `normalize_division()` - 16 tests for division name standardization (N1-N3, R1-R3, PR, PN, D1-D4)
  - `extract_team_id()` - 12 tests for team ID generation with gender markers (e.g., "1G", "3F") including detection of both "Dames" and "Feminine" keywords
